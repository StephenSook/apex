...
This document defines a defensible methods section for APEX/PhysicsTTM: a frozen time-series foundation model wrapped by a vehicle-physics projection layer that can be conditioned by adaptive-control metadata. The formulation is written for a NeurIPS workshop paper, with explicit assumptions, equations, implementation choices, evaluation metrics, ablations, and limitations.

## Methods claim to defend

APEX should claim that **PhysicsTTM projects raw forecasts from a frozen IBM Granite TimeSeries TTM model into a vehicle- and adaptation-aware feasible set**, rather than claiming that TTM itself understands vehicle physics. Tiny Time Mixer is a compact MLP-Mixer time-series foundation model with variants up to 1536 context steps in the r2 release, and its official model card describes channel-independent pretraining with optional channel-mixing fine-tuning rather than hard physical constraints ([TTM arXiv paper](https://arxiv.org/abs/2401.03955), [Granite TTM r2 model card](https://huggingface.co/ibm-granite/granite-timeseries-ttm-r2)).

The defensible novelty is the composition: **frozen TSFM forecast + post-hoc differentiable projection + motorsport vehicle-dynamics constraints + COA-derived adaptive-control semantics**. This does not claim that differentiable optimization layers are new, because OptNet introduced QP layers in neural networks and cvxpylayers generalized differentiable convex optimization layers to disciplined convex programs ([OptNet](https://arxiv.org/abs/1703.00443), [cvxpylayers](https://arxiv.org/abs/1910.12430)).
...
The evaluation reports standard forecast error and physical validity. Standard metrics include MAE and MSE. Physical metrics include constraint violation rate, projection distance, feasibility rate, post-projection accuracy degradation, and adaptive brake-throttle false positive and false negative rates. The central hypothesis is that PhysicsTTM sharply reduces constraint violations and adaptive brake-throttle false positives while producing only small post-projection accuracy degradation.

## Recommended next file tasks

1. Add this document to `docs/physics-ttm-constraint-spec.md` or `paper/physics-ttm-methods.md`.
2. Implement the QP version first, not the SOCP version.
3. Create the four synthetic fixtures F1 to F4.
4. Add `logs/day-01-latency-bench.md` for TTM and Granite 4.1 8B latency.
5. Add `logs/day-physics-projection-fixtures.md` with CVR, PD, FR, PPAD, BT-FP, and BT-FN results.
