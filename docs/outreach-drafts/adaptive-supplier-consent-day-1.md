...
Primary: [supplier contact, verified Day 2 via supplier site /contact before send]
...
APEX uses the FIA Certificate of Adaptations under the Appendix L provisions as a tensor-level input. When a driver's COA permits simultaneous brake-throttle inputs, our Stage 2 feasibility filter (which audits the nonconvex constraints downstream of a differentiable convex QP that handles friction-ellipse + forward-Euler + jerk-bound) recognises the COA gate. That is the architectural detail that distinguishes APEX from existing commercial race-engineer AI tools we surveyed (Track Titan, Trophi.ai). Public documentation for these tools does not show any conditional removal of the able-bodied brake-throttle mutual-exclusion assumption nor any FIA-Certificate-of-Adaptations parsing path; without such handling, simultaneous brake-throttle inputs read as either invalid telemetry or driver error rather than the COA-permitted technique they actually are.
...
If any of these surfaces would prefer anonymization ("leading UK adaptive-hand-control supplier") or different framing, I will adjust before the 2026-05-31 submission and will not name you on that surface.
...
Thank you for the time. The adaptive-racer community deserves better AI tooling than what currently exists. APEX is our 12-day attempt at that bar, with the IBM Granite stack underneath.
...
- [ ] Verify primary contact email via supplier site /contact before send.
...
- [ ] If reply consents: keep current persona materials with named attribution.
...
Replace specific supplier name with one of:
- (Hero / READMEs) "a leading UK adaptive hand-control supplier"
- (Video voiceover) "her electronic hand controls"
- (BeMyApp Story) "the kind of electronic hand-control system that adaptive drivers in UK championships actually use"
