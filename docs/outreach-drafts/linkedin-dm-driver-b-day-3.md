...
The short version: APEX reads a driver's telemetry, their FIA Certificate of Adaptations, and a written debrief, then produces a corner-by-corner coaching report. The architecture is the same IBM Granite stack IBM ships to Scuderia Ferrari's fans, plus a physics-projection layer that reads the COA at tensor level so the system permits simultaneous brake+throttle inputs when the adaptation permits it. Most existing AI race-engineer tools (Track Titan, Trophi.ai) misdiagnose adaptive technique as driver error because their physics models assume `throttle * brake = 0`. We fix that.
