...
Brake-pressure trace shows her hand-lever brake travel maxing at 38 mm with a release-rate that is 0.4 s earlier than her T7 reference lap at Croft. The mechanism: at full lever travel, the secondary actuation point requires a wrist-pivot that her amputation-side prosthetic stub loads awkwardly. She is effectively trail-braking against herself. APEX's two-stage projection-and-audit layer processes her forecast: Stage 1's convex QP keeps the friction-ellipse + forward-Euler + jerk-bound constraints satisfied, and Stage 2's feasibility filter reads her COA-derived c_overlap flag (APEX derives this flag from the approved hand-control hardware specifications recorded in Section 3(c) of her synthetic COA; the flag is set true because her MME Motorsport dual-stage trigger pattern is documented in that hardware spec as permitting simultaneous brake-throttle actuation) before clearing the brake-throttle overlap. APEX then recommends a 4 mm hand-lever brake-travel reduction at the secondary actuation point. That recommendation cites Appendix L (the FIA regulation governing COA structure for adaptive-driver homologation) + the approved hand-control hardware specifications recorded in Section 3(c) of her specific synthetic COA (the document from which APEX derives her c_overlap flag). The forecast envelope shows her recovering 0.42 s in Sector 2 next qualifying if she applies the tuning delta.
...
The failure-point row at Apex Old Hairpin shows brake (1.8 MPa) AND throttle (0.18, non-zero) present simultaneously. Standard tools hard-code `throttle * brake = 0`; they flag this row as invalid telemetry or driver error, the same way they would flag an able-bodied driver left-foot-braking. APEX runs Stage 1 of its projection-and-audit layer (the convex QP confirms peak lat-G of 0.81 + long-G of -0.22 sits within the constant-mu friction ellipse) and Stage 2 (the post-projection feasibility filter reads the COA-derived c_overlap flag, which APEX derived from the MME Motorsport dual-stage trigger hardware specification recorded in Section 3(c) of her synthetic COA, and audits the bicycle-model coupling for the resulting trajectory), then recommends the tuning delta instead of flagging the input.
...
1. **Corner-by-corner coaching report.** Three corners flagged with delta-vs-reference + recommendation prose + citation chip (FIA Appendix L for the COA-structure anchor + Section 3(c) of her synthetic COA + derived `c_overlap` flag from the approved hand-control hardware specifications in that section).
2. **Tuning recommendation card.** Reduce hand-lever brake travel by 4 mm. Citation chip surfaces the FIA Appendix L regulatory anchor + Section 3(c) of her synthetic COA + the `c_overlap` flag derived from the approved hardware spec. Provenance footer shows the Granite Guardian audit ID + model versions + commit SHA + COA section IDs.
3. **Next-session forecast chart.** 10 mini-sector envelope projection. Confidence band reflects the two-stage projection-and-audit layer's residual uncertainty after Stage 1's convex QP has projected the forecast onto the friction-ellipse + forward-Euler + jerk feasible set and Stage 2's feasibility filter has cleared the bicycle-model coupling + COA-simultaneity gate.
