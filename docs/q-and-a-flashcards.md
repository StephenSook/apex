...
**A:** Because public documentation for both tools, which we surveyed through 2026-Q2, does not show any conditional removal of the brake-throttle mutual-exclusion assumption. Simultaneous brake and throttle is a real racing technique (left-foot braking, trail-braking, holding throttle to keep a turbo spooled), and for adaptive drivers it is a homologated part of how their hand-control equipment works. We also found no public reference in either platform's docs to FIA Certificates of Adaptations as a parsed input. That's not a bug, it's the adaptation. Without explicit COA handling, the technique reads as either invalid telemetry or driver error. APEX reads the FIA Certificate of Adaptations as a flag at the tensor level. We're not coaching disabled drivers with the same model as everyone else. We're coaching them with the model that matches their car.
...
For questions outside the cards: "Wave-30 locked the maximal architecture: 8-tier physics + three-track forecasting ensemble + fifteen-tool IBM Granite stack (twelve through wave-30 D-016 + wave-45 D-054 baseline; wave-46 D-058 added Granite 4.1 3B Instruct fast-path routing + Granite Speech 4.1 2B-Plus ASR + Mellea IVR loop) + LangGraph + Granite MCP Gateway + ContextForge orchestration runtime + tri-agent Agent-as-Judge critic loop + seven shouldn't-be-possible moves (WebGPU Granite Nano + Activated LoRA hot-swap + GEPA reflective prompt evolution + EAGLE-3 speculative decoding + Agent-as-Judge tri-agent critic + wave-44 D-049 IBM TSPulse polyphase anomaly + wave-45 D-053 Granite TTM in-browser scaffold). All seven are in-scope; no V2 or V3 labels. The bounded-scope statement at paper §5.3 + the architecture spec at `docs/architecture-spec.md` Appendix W30 list the explicit non-claims. Happy to walk through where your question intersects the in-scope galaxy."
...
"Wave-30 D-019 enumerated five shouldn't-be-possible moves; wave-44 D-049 added IBM TSPulse polyphase anomaly detector as #6; wave-45 D-053 added Granite TTM in-browser scaffold as #7 (upstream-pipeline-gated per D-056 reframing). Move #1 WebGPU Granite Nano 350M edge inference via Transformers.js v4 shipped wave-38 (live on /judges + on /analyze with the 30-line Newton friction-ellipse projector for the offline path per D-021 server-authoritative reconnect). Wave-40 shipped the visualization surfaces for moves #2 (Activated LoRA hot-swap adapter status badge with rank + alpha + lambda + swap-round-trip-ms), #3 (GEPA reflective prompt evolution panel with candidate count + APEX-Bench faithfulness p95 trajectory over N iterations + per-iteration selected prompt ID), #4 (EAGLE-3 speculative decoding latency badge with speedup + accepted-token-rate + draft rank), and #5 polish on the tri-agent Agent-as-Judge critic loop via a new useTriAgentCriticVerdict data-source hook so the Day 7-8 backend wire-up is a fetch swap, not a build-from-zero. Moves #2 through #4 render in /judges#galaxy-moves between the 8-tier extended-physics grid + the tri-agent critic section; move #5 renders in the following tri-agent section; move #1 renders in /judges#edge-summary further down. Move #6 IBM TSPulse renders in /judges#galaxy-moves wave-44 cluster; move #7 TTM in-browser scaffold renders in /judges#ttm-in-browser with the canned-fallback runtime tier badge per D-056."
...
**Why this lands.** Reframes "fifteen tools" → "five end-to-end live + seven honest integration + three wave-46 D-058 additions (Granite 4.1 3B Instruct fast-path routing + Granite Speech 4.1 2B-Plus ASR + Mellea IVR loop)" instead of letting the judge feel ambushed by the tier markers. Names exactly which tools are live + cites the consumer file so the judge can verify in 30 seconds.
