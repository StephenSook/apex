...
**A:** That's exactly the problem PhysicsTTM solves. Every TTM forecast step passes through a differentiable QP enforcing three hard constraints: friction ellipse (total acceleration bounded by mu times g), forward-Euler kinematic step tying speed to longitudinal acceleration, and bicycle model tying lateral G to steering angle and speed. Lateral G with zero steering is infeasible by construction. V1 uses constant-mu, not full Pacejka. Sub-second hallucinations inside 1-Hz aggregates remain V2 work. The worst zero-order impossibilities never reach the driver.
