...
**At a glance.** 14 IBM Granite tools (3 wired, 9 integrated, 2 accelerators) · 192 backend tests in CI · 12 public pages and 20 API routes live · WCAG 2.1 AA · Apache 2.0, public from day one.
...
- **Wired end-to-end at HEAD (verified live in production).** The `/analyze` corner-by-corner coaching narrative is generated live by Granite 4.1 8B Instruct (via OpenRouter) from the driver's written debrief, and the same model powers the chat. Both verified responding at `phase=real` in production (wave-64 + wave-66). Plus the Granite 4.0 Nano 350M in-browser WebGPU edge model, the interactive COA simultaneity-gate toggle, and the engine-agnostic byte-equality projector demo, all running client-side in the browser.
...
- **Honest fallbacks, never faked numbers.** The `/analyze` coaching prose is live Granite; the telemetry deltas, forecast envelope, and tuning numbers shown alongside it remain a documented physics / backend swap-point (the model writes coaching, never telemetry, and never asserts FIA article numbers, enforced by a server-side scrubber). If the live Granite call is unavailable, the coaching degrades to an authored fixture narrative labelled "fixture" on the report, so the rendered provenance always tells the truth. The production-observability cockpit shows real Honeycomb spans when the backend is reachable and a clearly labelled wiring state when it is not. Nothing is presented as live that is not.
...
The 14-tool inventory (per D-058 expansion 2026-05-25 + wave-46-final IBM Bob removal 2026-05-26) carries a wire-up tier per tool surfaced as a status pill on the /judges page and the / page StackBadges grid (wave-44 Phase 4 BLOCKER 4 honesty audit). Three wired at HEAD (Granite Instruct 4.1 8B coaching narration + Granite 4.0 Nano 350M WebGPU edge model + Granite Embedding R2 via HF Inference Providers for live dense RAG rerank). Nine at integration with canonical type contracts + backend swap-points (Granite-Docling 258M + Granite Vision 4.1 4B + Granite TimeSeries TTM r2.1 + Granite FlowState r1.1 18.5M + IBM TSPulse 1M + Granite Guardian 4.1 8B + Granite Instruct 4.1 3B chat-routing + Granite Speech 4.1 2B-Plus Watson STT proxy preview + LangGraph + Granite MCP Gateway + ContextForge as the orchestration runtime per D-017 G7 + D-054; Langflow retained as the export-graph artifact). Two build-time accelerators (Docling library + Mellea v0.5.0 IVR-loop architectural slot; Mellea is documented as build-time architectural inspiration only, not a runtime dependency).
