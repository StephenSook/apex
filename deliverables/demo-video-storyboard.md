...
| 0:20 - 0:25 | SVG racing line crossfades to a static FIA Appendix L cover-page mockup with Article 18.3 highlighted. | "After the FIA lifted its single-seater ban on disabled drivers in December 2017, the barrier stopped being regulatory. It became economic." | Overlay: `2017-12 - FIA Article 18.3` | 400 ms cross-fade |
...
| Time | Visual | Audio (VO) | On-screen overlay | Cut |
|------|--------|-----------|-------------------|-----|
| 1:30 - 1:38 | Side-by-side split. Left: Sarah's COA Section 3(c) text excerpt on cream paper. Right: telemetry trace with brake + throttle traces overlaid simultaneously through the Old Hairpin window. | "Three firsts. First pretrained time-series foundation model on motorsport telemetry. First integrated workflow for adaptive hand-controls. First COA-parameterized brake-throttle simultaneity gate." | Three sequential lower-third overlays, one per "First", 1 s each, in Fraunces italic | Hard split-cut |
...
| 1:46 - 2:00 | Zoom into the COA Section 3(c) text. Highlight the simultaneity permission language. Cut back to telemetry trace with APEX permitted-region shading. | "So when a tool like Track Titan sees her telemetry, it flags her technique as driver error. APEX reads her Certificate of Adaptations at the tensor level. When her COA permits simultaneity, our physics layer permits it. We do not coach disabled drivers with the same model as everyone else. We coach them with the model that matches their car." | Final overlay: `COA at tensor level - feature, not bug` (racing green) | 800 ms zoom-in, then 600 ms cross-fade |
...
| 2:08 - 2:16 | Scroll the UI down to the tuning card. Highlight the brake-lever-travel delta and the COA section citation. | "a tuning recommendation reduce hand-lever brake travel by four millimetres citing the exact COA section," | Plex Mono callout: `38 mm -> 34 mm - Section 3(c)` | Animated scroll |
