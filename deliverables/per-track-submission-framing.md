...
- **Live Granite, verified.** The /analyze coaching narrative is generated live by Granite 4.1 8B Instruct (via OpenRouter), confirmed responding at `phase=real` in production. The "Run the canonical demo (live backend)" path runs the canonical telemetry through the deployed FastAPI backend's LangGraph pipeline (real physics-projected deltas, a deterministic Guardian rule-audit, a seasonal-naive forecast) and layers live Granite 4.1 8B corner coaching over those real backend numbers; it degrades to the backend's deterministic coaching if the narrate route is unreachable.
