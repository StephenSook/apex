...
- [ ] **Q-007 - APEX Lite EARLY trigger (Vinh-unresponsive scenario + Day-3 gate failure):** Distinct from Q-004's LATE Day-10 trigger (Day 9 dress-rehearsal failure). **Canonical decision rule (wave-30 revised):** Lite EARLY fires if ANY of {no Vinh reply in 48h, **D-027 SCP go/no-go gate fail AND fallback ladder exhausted** (per `docs/decision-log.md` D-027), G1 fail, G1b fail, G1c fail (FlowState + Chronos-2 zero-shot smoke), G4 fail, G6.5 fail, **Sync Point 1 fail** (Day 1-2 data contract lock)} holds. G0 autograd-spike retired per D-027 supersession. Invite-acceptance state is explicitly NOT a trigger condition pre-Tuesday 2026-05-27 EOD because BeMyApp Discord clarified the rate-limited rollout schedule (full history in `docs/wave-22-cold-review-findings.md`); re-enter the trigger set if Tuesday EOD passes with no Vinh response. **Action on trigger:** Stephen invokes APEX Lite per `docs/apex-lite-contingency.md` and logs the decision in `docs/decision-log.md` as `D-Lite-EARLY`. **Ship-floor on activation:** V1 NumPy physics validator + frozen Granite TTM r2.1 + Granite Guardian 4.1 text-audit baseline per D-A wave-25 architecture (same ship-floor as Q-004; Lite is one named fallback referenced from both triggers). **Re-evaluation cadence:** every 12 hours through Day 4 EOD; Day 5 onward the trigger window collapses into Q-004 only.
