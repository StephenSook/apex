...
**Critical-path Vinh deps still open (Day 5):**
1. ~~Phase 1 task 1.1 Granite-Docling COA parser~~ ✅ shipped Day 4 commit `e9c9d11` (JSON-first rung-0; Docling PDF rung deferred to Phase 2 per `logs/day-04-docling-bench.md`)
2. ~~Phase 1 task 1.7 OpenRouter sample call~~ ✅ already-shipped Stephen-side wave-42 (commits `7179dc1` + `89da292` + `dc5bd7e`); Vinh consumes via frontend route per `docs/vinh-phase-1-handoff.md` Q3 split.
3. ~~Phase 2 Day 4 G3 block (tasks 2.1-2.7)~~ ✅ PASS Day 4 commit `9e114b5` (V1 NumPy validator + 19 tests + Convergence-14 floor + `logs/day-04-g3.md`).
4. ~~Phase 2 Day 4 task 2.8 TTM wrapper~~ ✅ Day 4 commit `373a882` (15 tests).
5. ~~Phase 2 Day 4 task 2.9 telemetry pipeline~~ ✅ Day 4 commit `745e4cf` (11 tests + CLI).
6. ~~Phase 2 Day 4 task 2.10 FastF1 integration test~~ ✅ Day 4 commit `a157fb6` (5 tests + --integration flag).
7. **Phase 2 Day 4 Gate G4 FAIL** Day 4 (`logs/day-04-g4.md`): TTM zero-shot loses to seasonal-naive on speed_mps by ~2x. Plan L377 fine-tune-first pivot triggered. Action items: pitch language correction (drop "zero-shot beats naive" claim Cards 03 + 04), elevate D-010 Track 1 channel-mix decoder fine-tune to Day 5 morning track, reframe TTM as forecast-envelope generator. Discord 9 PM ET sync escalation due tonight.
8. Phase 2 Day 5 task 2.12 SCP expansion (cvxpylayers V2 projector + 8-tier Pacejka + 3-iteration unroll per D-031; Stage A + Stage B of the staged D-027 spec).
9. Phase 2 Day 5 task 2.13-2.16 (V2 cut decision + Granite Guardian audit + Gate G5).
