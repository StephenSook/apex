...
**Phase 1 - Document parsing (Day 4-5, Vinh):** 🟡 in progress. Vinh task 1.1-1.6 + 1.9 COA parsing + simultaneity-flag extraction ownership (Granite-Docling 258M pre-fetched Day 3, 505 MiB cached); Stephen task 1.7 OpenRouter Granite API plumbing (wave-42 commit 7179dc1) + 1.8 streaming-response handler (wave-42 commit dc5bd7e) shipped. Sarah Reynolds COA fixture stub shipped wave-42 (commit 82d1f85). Stream M.3 spec handoff at docs/wave-41-backend-spec-handoff.md.
...
**Phase 5 - Demo + deploy (Days 9-10, both):** 🟡 mid-flight. apex-one-black.vercel.app Vercel production deploy live (wave-38 cutover). Colab walkthrough at deliverables/apex-demo.ipynb. /judges + /analyze + /status + /sim-rig all live. 3-min demo video script + 11-frame deck PDF + 30-second highlight clip storyboard pending wave-43 Lane C2.J. HF Space + sim-rig WebSocket backend + production video record Day 10-11.
...
**Critical-path Vinh deps still open (Day 4):**
1. Phase 1 task 1.1 Granite-Docling COA parser (gated on 9 PM Day-3 Stephen sync: COA fixture source decision)
2. Phase 1 task 1.7 OpenRouter sample call (gated on 9 PM Day-3 Stephen sync: API key path)
3. Phase 2 Day 4 task 2.12 SCP expansion (8-tier Pacejka + 3-iteration unroll per D-031; Stage A + Stage B of the staged D-027 spec)
4. Gate G4 Day-4 zero-shot TTM vs seasonal-naive on FastF1 holdouts (FastF1 cache hot Day 3)
...
|---|-----------|---------|-------|--------|------|-------|
| 1.1 | Granite-Docling 258M parses FIA COA PDF into structured JSON | `app/backend/apex/intake/coa_parser.py` | Vinh | ⬜ | 0.13 ✅; **1.3 (COA fixture source) gated on Stephen 9 PM Day-3 sync** | Preserve section IDs + 9 adaptation domains. Granite-Docling 258M pre-fetched Day 3 (505 MiB cached). |
...
| 1.3 | Fixture COA + timing-sheet committed | `fixtures/coa/`, `fixtures/timing-sheets/` | Vinh | ⬜ | 1.1, 1.2; **fixture source decision gated on Stephen 9 PM Day-3 sync** | Public FIA + SRO examples, OR synthetic Sarah COA (Vinh-authored), OR MME-anonymized sample. Pick at sync. |
...
| 1.9 | OpenRouter API key + Granite 4.1 8B sample call (Discord intel 2026-05-20: OpenRouter approved, free tier, no CC required, `openrouter.ai/ibm-granite/granite-4.1-8b`) | logs entry + `.env.example` | Vinh | ⬜ | 0.13 ✅; **OpenRouter + watsonx.ai account stand-up gated on Stephen 9 PM Day-3 sync** | Primary inference path for OpenRouter deploy per D-019 / plan task 5.1. Backup = watsonx.ai. Vinh has neither account; ask Stephen to stand up both + send `.env` entries. Local llama.cpp Granite 4.1 8B Q4_K_M Day-3 baseline at 7.1 tok/s (G1b commit `b61c629`) is offline-fallback only. |
...
| 2.9a | SCP inner-iterate convex QP via cvxpylayers (friction ellipse + forward-Euler kinematic + jerk bound; per D-012 unrolled SCP outer loop) | `app/backend/apex/physics/projection.py` | Vinh | ⬜ | 2.5, **D-027** | Day 5 (gated on D-027 SCP go/no-go gate Day 3). Convex constraints only; decision variables `(a_long, a_lat, speed_{t+1})`. Wave-30 D-012 reframes 2.9a as the convex QP INNER ITERATE of an outer 3-iteration unrolled SCP loop (8-tier non-convex physics handled via Taylor linearization around previous iterate fed back into this inner solve). Jerk-bound activates at >=10 Hz per Rajamani (vehicle-dynamics standard); C14-04 1 Hz fixture is a deliberate hackathon-demo simplification, not the production path. See `docs/wave-24-cold-review-findings.md` for the §3.2 reformulation rationale + `docs/wave-25-cold-review-findings.md` for the SI-unit convention + `docs/wave-28-cold-review-findings.md` H-W28-5 + arch-spec jerk_max sampling-rate caveat for the >=10Hz production trajectory + `docs/decision-log.md` D-012 + D-027 for the wave-30 SCP outer-loop reframing. |
| 2.9b | SCP outer-loop Taylor linearization of 8-tier non-convex physics (Pacejka combined-slip + transient tire + thermal + load transfer + double-track + 3D track + aero + adaptive hand-controls; per D-012) + non-differentiable feasibility audit (bicycle-model + COA-parameterized simultaneity gate; per D-A wave-25 legacy) | `app/backend/apex/physics/scp_outer.py` (new wave-30) + `app/backend/apex/physics/feasibility.py` (D-A wave-25 legacy) | Vinh | ⬜ | 2.9a, **D-027** | Day 5 (gated on D-027 SCP go/no-go gate Day 3). Wave-30 D-012 promotes the wave-25 two-stage projection-and-audit into an unrolled SCP outer loop: 8 non-convex physics tiers get first-order Taylor linearization around the previous iterate + fed back into 2.9a inner solve. The D-A wave-25 non-differentiable feasibility filter (bicycle + COA-simultaneity) survives as a Guardian audit-side check (post-SCP-solve), running alongside the differentiable outer loop. **Threshold lock** (committed before 2.9a closes): slip-tolerant bound = 0.03 rad; ε numerical tolerance = 1e-6. SCP outer iterations: 3 fixed per D-012 (empirical-gated by D-027 prototype confirmation). |
| 2.9c | Stage 1 + Stage 2 unit-test suite | `app/backend/tests/test_projection.py` + `app/backend/tests/test_feasibility.py` | Vinh | ⬜ | 2.9a, 2.9b | Day 5. Stage 1 projects 10 fixture tensors within ε=1e-4 of analytic optimum; Stage 2 rejects bicycle-violating fixtures + COA-simul-violating fixtures with 100% recall on the Convergence-14 fixture enumeration. C14-04 jerk-bound fixture is honest-demo-only (1 Hz aggregation); production V2 path runs jerk-bound at >=10 Hz on the raw 50 Hz upstream signal per `docs/architecture-spec.md` jerk_max sampling-rate caveat + `paper/physics-ttm-methods.md` §4 (to land per row 6.7c). |
