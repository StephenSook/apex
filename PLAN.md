...
| 1.14 | Verify Vinh receives + accepts the same GitHub org invite under his account | external (Vinh's email) | Vinh | ⬜ | 1.13 | Per the rules, team-member info ships on the BeMyApp form. Vinh-side org membership is not a binding requirement, but it unlocks the 3 private repos (rules + submission + showcase) for his reference. |
...
| 5.14 | BeMyApp 1920x600 banner asset (editorial-paddock palette + APEX wordmark + tagline + hero visual) | `deliverables/bemyapp-banner-1920x600.png` + renderer at `app/frontend/lib/bemyapp-banner.tsx` (Next ImageResponse) | Stephen | ⬜ | - | Pulled forward from Phase 7 to Day 2-3. Mirrors the live BeMyApp project page template (reference: `reference_bemyapp_submission_template.md`). Cinematic 16:5 aspect. Render via Next.js ImageResponse for type-safe version control + auto rebuild on commit. |
