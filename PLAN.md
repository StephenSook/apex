...
**Commits Day 1 + Day 2:** 197+ atomic commits, all pushed, CI green per push (concurrency block dropped wave-18 to prevent middle-queue cancel cascade).
...
- `docs/wave-24-cold-review-findings.md` - consolidated wave-24 findings (9 BLOCKER + 14 HIGH + 12 MED + 2 NIT). Defines wave-24 closure IDs; closures land in wave-25 commits.
...
| 1.14 | Verify Vinh receives + accepts the same GitHub org invite under his account | external (Vinh's email) | Vinh | ⬜ | 1.13 | Per the rules, team-member info ships on the BeMyApp form. Vinh-side org membership is not a binding requirement, but it unlocks the 3 private repos (rules + submission + showcase) for his reference. **Update 2026-05-21 night per Discord intel from `Lucas-BMA`:** invites are throttled at 500/day; full participant rollout completes by Tuesday 2026-05-26 to 2026-05-27. Vinh-side absence is benign + the `hands-on-labs` repo is already public so Vinh can start TORCS without org access (PLAN row 1.10 recommended-only). Re-check Tuesday 2026-05-27 EOD; if no Vinh response by then, evaluate Q-007 EARLY APEX Lite trigger on the OTHER failure modes (no reply in 48h, G1/G4 fail). |
...
| 1.16 | Ask Discord whether ONE submission can win multiple per-challenge awards (1st + Best Use of Technology + Most Innovative stacking) | discord post | Stephen | ✅ | 1.15 | Resolved 2026-05-21 night via Discord answer captured in Stephen-shared screenshot. `Lucas-BMA` answered `Chi blú [GTLB]`: "You're submitting 1 project and prizes such as best use of technology are decided by the judges!" **Verdict: NO opt-in tracks.** ONE submission per team. Judges award the four per-challenge categories on their own assessment. Multi-track-stacking strategy from global CLAUDE.md is mooted; project-flow falls back to: build the best single submission across all four official judging criteria (Technical Execution / Innovation / Challenge Fit / Implementation & Feasibility). Cross-reference: `reference_competitors_calibration.md`. |
...
| 2.9a | Stage 1 convex QP projection (friction ellipse + forward-Euler kinematic + jerk bound) via CvxpyLayer | `app/backend/apex/physics/projection.py` | Vinh | ⬜ | 2.5 | Day 5. Convex constraints only (decision variables `(a_long, a_lat, speed_{t+1})` with TTM forecast as exogenous prior-step state). Bicycle + COA-simultaneity gate are NOT in the QP - they move to row 2.9b per wave-24 cold-review B1+B2 closure (paper §3.2 reformulation). |
| 2.9b | Stage 2 post-projection feasibility filter (bicycle-model audit + COA-parameterized simultaneity gate) | `app/backend/apex/physics/feasibility.py` | Vinh | ⬜ | 2.9a | Day 5. Non-differentiable accept/reject audit on the Stage-1 projected tensor. Bicycle model as low-slip kinematic approximation (`steering_rad` defined as road-wheel angle); COA-simultaneity gate as complementarity check (nonconvex, cannot live in the convex QP per CvxpyLayer's mixed-integer limitation). Specific thresholds (slip-tolerant bound + ε numerical tolerance) committed at Gate G5 land; reported in paper §4.1 at camera-ready. |
...
| 6.7 | `paper/apex-neurips-workshop-2026.md` DRAFT (readable quality) | paper/ | Stephen + Vinh | 🟡 | 4.2, 5.7 | Publication-readable draft ✅ wave-23 + wave-24 closures Day 2 night-late-late: Plain-language summary + 250-word Abstract + §1 Introduction + §2 Related Work + §3 Method (Stage-1 convex QP + Stage-2 post-projection feasibility filter per wave-23 cold-review B1+B2 reformulation) + §5 Limitations + §5.2 Threats to validity + §5.3 bounded-scope claims + §6 Conclusion (demoted to hypothesis-with-caveat post-wave-23 B4) + §7 Reproducibility (tense-shifted to forward-looking with PLAN-row pointers post-wave-23 B3) + §8 Ethics (IRB + informed-consent forward statement post-wave-23 H8) + §9 Acknowledgments + §10 Author contributions + §11 COI + §12 Funding + §13 References (Chronos misattribution fixed post-wave-23 H1; Deep Dynamics + CvxpyLayer + Granite TTM all cite verified arXiv/proceedings sources; remaining `(cite Day 11)` placeholders removed). Vinh fills §4 Experiments Day 9-10 once benchmarks land. **Acceptance criterion:** Day 11 final draft is readable by an external researcher in one pass; passes (a) Abstract <=250 words, (b) §3 Method has Layer 1+2+3 math equations + QP convexity correctly identified, (c) §4 Experiments has at least one benchmark (TTM-zero-shot RMSE vs seasonal-naive on at least 1 FastF1 holdout), (d) §5 Limitations honest about COA fixture synthesis + retraining-free claim scope, (e) References has at least 10 cited works including the Granite-TTM NeurIPS 2024 paper. |
| 6.8 | README.md final polish | repo root | Stephen | 🟡 | 5.8 | Wave-23 Day 2 night-late-late pull-forward: BeMyApp banner embedded at top via relative path; status snapshot bumped from Day-1 placeholders to actual Day-2-night per-phase Stephen+Vinh state (with wave-24 cold-review fixes for Phase 0 emoji + HF Space badge "Coming Day 9" + calibration-ceiling line removed + commit count to 194+ + price-claim soften per wave-23 MED M4). Day 11 final polish remaining = real demo URL + Cloud Run links + final AI-tone re-sweep + final commit-count bump. |
...
- [ ] **Q-007 - APEX Lite EARLY trigger (Vinh-unresponsive scenario):** Distinct from Q-004's LATE Day-10 trigger (Day 9 dress-rehearsal failure). Wave-22 cold-review WARN #1-3 cleaned up the decision rule after the Discord intel landed: invite-acceptance is NOT a trigger condition before Tuesday 2026-05-27 EOD because `Lucas-BMA` confirmed GitHub-org invites are rate-limited at 500/day with full rollout by then + the `hands-on-labs` repo is already public so Vinh can start TORCS without org access. **Canonical decision rule:** Lite EARLY fires if ANY of {no Vinh reply in 48h, G1 fail, G4 fail} holds. Invite-acceptance state is explicitly NOT a trigger condition pre-Tuesday 2026-05-27 EOD; re-enter the trigger set if Tuesday EOD passes with no Vinh response. **Action on trigger:** Stephen invokes APEX Lite per `docs/apex-lite-contingency.md` and logs the decision in `docs/decision-log.md` as `D-Lite-EARLY`. **Re-evaluation cadence:** every 12 hours through Day 4 EOD; Day 5 onward the trigger window collapses into Q-004 only.
...
_Last updated: 2026-05-21 night-late-late-4 by Stephen after 25 review waves (waves 1-18 Day 1 + waves 19-25 Day 2). **197+ atomic commits pushed across Day 1 + Day 2.** Phase 0 ✅ except 0.11-0.13 (Vinh-side; Tuesday 2026-05-27 ETA per Discord intel). Wave-19 mega-batch (paper outline + sim-rig + Colab + storyboard + Vercel) + wave-20 (IBM-SkillsBuild org invite + Discord + Official Rules WebFetch + BeMyApp template + per-route OG cards + privacy fix) + wave-21 (4-competitor calibration + Discord verdict + Tuesday ETA + panel-depth rewrite) + wave-22 (BeMyApp 1920x600 banner + cold-review closures 5 BLOCKER + 14 HIGH) + wave-23 (NeurIPS paper expansion + MED closures + README polish) + wave-24 (wave-23 cold-review closures 5 BLOCKER + 8 HIGH + paper §3.2 QP convexity reformulation + Chronos citation fix + Ethics IRB statement + Acknowledgments + paper completeness gaps closed + brand-fonts comment correctness + README polish). Day 2 + Day 4 + Day 5 + Day 6 + Day 7 + Day 8 + Day 9 + Day 10 + Day 11 task drafts pulled forward per galaxy-tier rule (Core 6 + Stretch S1 + S4 + S5 + S6 + S10 all at draft quality). Pre-consent operator-attribution anonymized across 14+ public files. plan-gap-scanner BLOCKERs closed across waves 19-22-23. APEX Lite EARLY Q-007 trigger softened per Discord intel; re-evaluation Tuesday 2026-05-27 EOD if Vinh still dark._
