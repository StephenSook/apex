...
| 6.7 | `paper/apex-neurips-workshop-2026.md` DRAFT (readable quality) | paper/ | Stephen owns §1-§3 + §5-§13; Vinh owns §4 Experiments | 🟡 | 4.2, 5.7 | Publication-readable draft of §1-§3 + §5-§13 ✅ wave-23 + wave-24 closures (consolidated into `docs/wave-24-cold-review-findings.md`); §4 Experiments scheduled Day 9-10 once Vinh-side benchmarks land per PLAN.md §Sources of truth Cold review findings index. **Acceptance criterion:** Day 11 final draft is readable by an external researcher in one pass; passes (a) Abstract <=250 words, (b) §3 Method has all three layers' math equations + QP convexity correctly identified (Stage-1 convex QP + Stage-2 post-projection feasibility filter), (c) §4 Experiments has at least one benchmark (TTM-zero-shot RMSE vs seasonal-naive on at least 1 FastF1 holdout), (d) §5 Limitations honest about COA fixture synthesis + retraining-free claim scope, (e) §13 References has at least 10 cited works including the Granite-TTM NeurIPS 2024 paper (verified arXiv/proceedings sources per Chrosniak et al. 2023 + Agrawal et al. 2019 + Ekambaram et al. 2024 citation convention). |
| 6.8 | README.md final polish | repo root | Stephen | 🟡 | 5.8 | Pull-forward through Day 2 night-late-late closures (full closure log in `docs/wave-24-cold-review-findings.md`): BeMyApp banner embedded at top via relative path; status snapshot bumped from Day-1 placeholders to actual Day-2-night per-phase Stephen+Vinh state; Phase 6 paper status flipped to 🟡; HF Space "hosted by submission week" tense softening; "transatlantic" qualifier removed; commit count synced to 197+. Day 11 final polish remaining: M6 first-claim heading rewrite mirroring paper §5.3 bounded scope + real demo URL + Cloud Run links + final AI-tone re-sweep + final commit-count bump. |
