...
3. Phase 2 Day 4 G3 block (tasks 2.1-2.7): V1 NumPy validator function bodies + violation log golden fixtures + 5-impossible/5-valid test grid + round-trip serializer assertion. Signatures already at `app/backend/apex/physics/validator.py` per Phase 0 task 0.11.
4. Phase 2 Day 4 G4 block (tasks 2.8-2.11): `ttm/forecast.py` 1Hz mini-sector aggregator + TTM zero-shot vs seasonal-naive holdout (laps 4-5, seed=42, channels speed_mps + long_g).
5. Phase 2 Day 4 task 2.12 SCP expansion (8-tier Pacejka + 3-iteration unroll per D-031; Stage A + Stage B of the staged D-027 spec).
